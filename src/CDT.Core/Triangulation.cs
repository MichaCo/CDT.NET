    ...
    where T : unmanaged, IFloatingPoint<T>, IMinMaxValue<T>, IRootFunctions<T>
{
    ...
    /// <summary>Inserts a span of vertices into the triangulation.</summary>
    public void InsertVertices(ReadOnlySpan<V2d<T>> newVertices)
    {
        ...
        _vertices.EnsureCapacity(_vertices.Count + newVertices.Length);
        _triangles.EnsureCapacity(_triangles.Count + 2 * newVertices.Length + 1);
        _vertTris.EnsureCapacity(_vertTris.Count + newVertices.Length);
    ...
    private void InsertVertices_Randomized(int superGeomVertCount)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < count; i++) { InsertVertex(indices[i]); }
        }
        finally
        {
    ...
    private void InsertVertices_KDTreeBFS(int superGeomVertCount, Box2d<T> box)
    {
        ...
        var indices = ArrayPool<int>.Shared.Rent(vertexCount);
        try
        {
    ...
    private (int iT, int iTopo) WalkingSearchTrianglesAt(int iVert, int startVertex)
    {
        ...
        var v = vertSpan[iVert];
        int iT = WalkTriangles(startVertex, v);
        ref readonly var t = ref CollectionsMarshal.AsSpan(_triangles)[iT];

        var loc = LocatePointTriangle(v, vertSpan[t.V0], vertSpan[t.V1], vertSpan[t.V2]);
        ...
        if (loc == PtTriLocation.Outside)
        {
            ...
            t = ref CollectionsMarshal.AsSpan(_triangles)[iT];
        }

        if (loc == PtTriLocation.OnVertex)
        {
            int iDupe = vertSpan[t.V0] == v ? t.V0 : vertSpan[t.V1] == v ? t.V1 : t.V2;
    ...
    private int FindTriangleLinear(V2d<T> pos, out PtTriLocation loc)
    {
        ...
        for (int i = 0; i < triSpan.Length; i++)
        {
            ref readonly var t = ref triSpan[i];
            loc = LocatePointTriangle(pos, vertSpan[t.V0], vertSpan[t.V1], vertSpan[t.V2]);
            if (loc != PtTriLocation.Outside)
            {
                return i;
            }
    ...
    private int WalkTriangles(int startVertex, V2d<T> pos)
    {
        ...
        {
            ref readonly var t = ref triSpan[currTri];
            bool found = true;
            int offset = guard % 3;
            for (int i = 0; i < 3; i++)
            {
                ...
                var loc = LocatePointLine(pos, vertSpan[vStart], vertSpan[vEnd]);
        ...
        out int n1, out int n2, out int n3, out int n4)
    {
        ...
        ref readonly var tOpo = ref triSpan[iTopo];
        ...
        int n1, int n2, int n3, int n4)
    {
        ...
        span[iT] = new Triangle(v4, v1, v3, n3, iTopo, n4);
        span[iTopo] = new Triangle(v2, v3, v1, n2, iT, n1);
        ...
        List<TriangulatePseudoPolygonTask> iterations)
    {
        ...
        // Finalize triangle
        {
            ref var parentTri = ref CollectionsMarshal.AsSpan(_triangles)[iParent];
            parentTri.SetNeighbor(iInParent, iT);
        }
        {
            ref var tFinal = ref CollectionsMarshal.AsSpan(_triangles)[iT];
            tFinal.N0 = iParent; tFinal.V0 = a; tFinal.V1 = b; tFinal.V2 = c;
        }
    ...
    private void ChangeNeighbor(int iT, int oldN, int newN)
    {
        ...
        ref var t = ref CollectionsMarshal.AsSpan(_triangles)[iT];
    ...
    private void RemoveTriangles(HashSet<int> removed)
    {
        ...
        var mapping = ArrayPool<int>.Shared.Rent(triCount);
        try
        {
            int newIdx = 0;
            ...
            // Compact triangle list using span (no per-element bounds checks)
            {
                var span = CollectionsMarshal.AsSpan(_triangles);
                int write = 0;
